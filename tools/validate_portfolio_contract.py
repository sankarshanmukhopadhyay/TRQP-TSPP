...
def main() -> int:
    ...
    checks.append(check(
        "semantic-authority",
        (semantic.get("repository"), semantic.get("version")) == EXPECTED_SEMANTIC and bool(semantic.get("artifacts")),
        "TSMM authority must be pinned to 0.24.0 with declared artifacts",
    ))
    checks.append(check(
        "schema-authority",
        (schema.get("repository"), schema.get("version")) == EXPECTED_SCHEMA and bool(schema.get("artifacts")),
        "TIS authority must be pinned to 0.14.1 with declared artifacts",
    ))
    ...
    checks.append(check(
        ...
        {"sankarshanmukhopadhyay/trqp-conformance-suite", "sankarshanmukhopadhyay/trqp-assurance-hub"} <= peers,
        "required TRQP peer relationships are declared",
    ))
