...
When used with the Assurance Hub Operational Stack baseline, operators MUST set `--run-id` and `--target-id` so TSPP emits metadata that matches the paired CTS report. Assurance Hub v1.6.1 rejects Combined Assurance Manifest generation when these fields drift.
